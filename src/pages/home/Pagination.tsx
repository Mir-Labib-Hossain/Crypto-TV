...
const Pagination = ({ currentPage, totalPage }: Props) => {
  ...
  for (let i = 1; i <= totalPage; i++) {
    paginations.push(
      <button onClick={() => dispatch(updatePage(i))} disabled={currentPage === i} className={`bg-yellow-${currentPage === i ? "400 text-white cursor-not-allowed" : "100 text-yellow-500 "} px-4 py-1 rounded-full cursor-pointer`} key={i}>
        {i}
      </button>
    );
  }
